		...
		submitHandler : function(form) {
			...
			$.post("pages/back/admin/travel/add_cost.action",{tpid:tpid,title:title,price:price,tid:tid},function(data){
				...
				if(data.status){
					$(title).val("") ;
					$(price).val("") ;
					costInfo="<tr id='travel-"+data.travelCost.tcid+"'>"+
									"<td class='text-center'>"+data.type.title+"</td>"+
									"<td class='text-center'>￥<span id='price-"+data.travelCost.tcid+"'>"+data.travelCost.price+"</span></td>"+
									"<td class='text-center'>"+data.travelCost.title+"</td>"+
									"<td class='text-center'>"+
										"<button class='btn btn-danger btn-xs' id='remove-"+data.travelCost.tcid+"'>"+
