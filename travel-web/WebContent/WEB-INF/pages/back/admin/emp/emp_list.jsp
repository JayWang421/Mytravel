			...
			<div class="panel-body">
				...
				<table class="table table-condensed">
					...
					<c:forEach items="${allEmps}" var="emp">
						<tr>
							<td class="text-center"><input type="checkbox" id="eid-${emp.ename}" value="${emp.eid}"></td>
							...
							<c:forEach items="${allDepts}" var="dept">
								<c:if test="${emp.did==dept.did}">
									<td class="text-center">${dept.dname}</td>
								</c:if>	
							</c:forEach>
							<c:forEach items="${allLevels}" var="level">
								<c:if test="${emp.lid==level.lid}">
									<td class="text-center">${level.title}</td>
								</c:if>	
							</c:forEach>
							<td class="text-center"><fmt:formatDate value="${emp.hiredate}" type="date"/></td>
